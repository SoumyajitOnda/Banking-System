...
public class ApproveCustomerServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASS);) {
			...
			if (updated > 0) {
				response.sendRedirect("pages/approve_customer.jsp?msg=success");
			} else {
				response.sendRedirect("pages/approve_customer.jsp?msg=error");
			}

			ps.close();
		} catch (Exception e) {
			e.printStackTrace();
			response.sendRedirect("pages/approve_customer.jsp?msg=exception");
		}
	}
}
